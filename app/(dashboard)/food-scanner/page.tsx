          ...
          <div className="max-w-6xl mx-auto">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-400 mt-0.5">Powered by Gemini Vision • Real-time food analysis</p>
                </div>
              </div>
            </motion.div>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
                className="rounded-2xl border border-white/10 bg-white/[0.03] backdrop-blur-xl p-6 relative overflow-hidden">
                ...
                {!capturedImage ? (
                  <>
                    <div className="relative rounded-xl overflow-hidden bg-black/50 mb-4 border border-white/10">
                    ...
                    <div className="flex gap-3">
                      <button onClick={capturePhoto}
                        className="flex-1 py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-medium flex items-center justify-center gap-2 hover:shadow-[0_0_20px_rgba(6,182,212,0.3)] transition-all">
                        <Camera className="w-5 h-5" /> Capture
                      </button>
                      <label className="flex-1 cursor-pointer">
                        <input type="file" accept="image/*" onChange={handleFileUpload} className="hidden" />
                        <div className="py-3 rounded-xl border border-white/10 bg-white/5 text-white font-medium flex items-center justify-center gap-2 hover:bg-white/10 transition-all">
                  ...
                  <>
                    <div className="relative rounded-xl overflow-hidden mb-4 border border-white/10">
                      ...
                      {result && (
                        <div className={`absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/80 to-transparent`}>
                          <p className="text-white font-bold text-lg">{result.foodName}</p>
                        </div>
                      )}
                    </div>
                    {error && (
                    ...
                    <button onClick={analyzeFood} disabled={isAnalyzing}
                      className="w-full py-3 rounded-xl bg-gradient-to-r from-cyan-500 to-purple-600 text-white font-medium flex items-center justify-center gap-2 disabled:opacity-50 hover:shadow-[0_0_20px_rgba(6,182,212,0.3)] transition-all">
                ...
                <AnimatePresence>
                  {isAnalyzing && (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                      className="rounded-2xl border border-cyan-500/20 bg-white/[0.03] backdrop-blur-xl p-8 text-center relative overflow-hidden">
                      ...
                      <div className="relative">
                        ...
                        <h3 className="text-xl font-semibold text-white mb-2">AI Analyzing Your Meal...</h3>
                        <p className="text-gray-400 text-sm">Identifying ingredients and nutritional content</p>
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                      ...
                      <div className="flex items-center justify-between mb-6">
                        <div>
                          ...
                          <p className="text-sm text-gray-400 mt-1">AI Analysis Complete</p>
                        </div>
                      ...
                      {/* Ingredients */}
                      <div className="mb-4">
                        <h4 className="text-white font-medium mb-2 text-sm">Detected Ingredients</h4>
                        <div className="flex flex-wrap gap-1.5">
                          {result.ingredients.map((ing, i) => (
                            <span key={i} className="px-2 py-1 rounded-full bg-white/10 text-gray-300 text-xs border border-white/5">{ing}</span>
                          ))}
                        </div>
                      </div>
                        ...
                        <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
                          <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${getScoreGradient(scan.healthScore)} flex items-center justify-center`}>
                              <span className="text-xs font-bold text-white">{scan.healthScore}</span>
                            </div>
                            <span className="text-white text-sm">{scan.foodName}</span>
                          </div>
